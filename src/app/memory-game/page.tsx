...
export default function MemoryGameSelect() {
  ...
  return (
    <div className="flex flex-col items-center justify-center min-h-screen bg-gradient-to-br from-yellow-200 to-pink-200 relative">
      ...
      <h1 className="text-4xl font-bold mb-8 text-yellow-700 drop-shadow-lg text-center">Memory Game</h1>
      <div className="grid grid-cols-1 md:grid-cols-2 gap-8 w-full max-w-xl">
        <div
          ...
          onClick={() => router.push("/memory-game/single")}
        >
          <img src="https://cdn-icons-png.flaticon.com/512/616/616554.png" alt="Single Player" className="w-20 h-20 mb-4" />
          <h2 className="text-2xl font-semibold mb-2 text-pink-700 text-center">Play Solo</h2>
        </div>
        <div
          className="bg-white rounded-2xl shadow-lg p-8 flex flex-col items-center hover:scale-105 transition cursor-pointer text-justify"
          onClick={() => router.push("/memory-game/friend")}
        >
